...
soundex = fuzzy.DMetaphone() # fuzzy.Soundex(10)
maxvalue = 75
...
def comparewithcsv(name,skip,idcompara):
    originalsoundx = soundex(name) #soundex.phonetics(name)
    results = []
    sdn=[]
    alias = 'a.k.a. '
                ...
                if int(row[0]) not in skip:
                    ...
                    if splitaka > 0:
                        ...
                        for i in akdata: 
                            if i.find(alias) > 0:
                                ...
                                comparename = comparationcompletename(name,akaname,originalsoundx[0].decode("utf-8"))
                                if validationScore(comparename[0],comparename[1]):
                                    results.append({'id':row[0],'nombre':row[1],'alias':akaname,'tipo':row[3],'jaro':comparename[0], 'sound':comparename[1]})
                                    guardaofac=1
                                    continue

                                rsjarowords = comparationbyword(name, akaname)
                                if validationScore(rsjarowords[1], rsjarowords[0]):
                    ...
                    if len(akadata) > 0:
                        for i in akadata: 
                            ...
                            comparename=comparationcompletename(name,akaname,originalsoundx[0].decode("utf-8"))
                            if validationScore(comparename[0], comparename[1]):
                                results.append({'id':row[0]+'-'+i[1] ,'nombre':row[1],'alias':akaname,'tipo':row[3],'jaro':comparename[0], 'sound':comparename[1]})
                                guardaofac=1
                                continue
                    ...
                    comparename=comparationcompletename(name,namecsv,originalsoundx[0].decode("utf-8"))
                    if validationScore(comparename[0],comparename[1]):
                        results.append({'id':row[0],'nombre':row[1],'alias':'','tipo':row[3],'jaro':comparename[0], 'sound':comparename[1]})
                        guardaofac=1
                        continue
...
def validationScore(jaro,sound):
    return jaro >= maxvalue and sound >= maxvalue

def comparationcompletename(name,compare,sound):
    presaundex  = soundex(compare.upper())
    print(presaundex)
    rsjaro      = jaro.jaro_winkler_metric(name.upper(),compare.upper())*100
    rssound     = jaro.jaro_winkler_metric(sound,presaundex[0].decode("utf-8"))*100
    return [rsjaro,rssound]


def comparationbyword(name,compare):
    lista1 = list(dict.fromkeys(name.upper().replace(',','').replace("'",'').split(' ')))
    lista2 = list(dict.fromkeys(compare.upper().replace(',','').replace("'",'').split(' ')))
    ...
    for a in lista1: 
        for b in lista2: 
            try:
                ...
                datosound = jaro.jaro_winkler_metric(sounda[0].decode("utf-8"),soundb[0].decode("utf-8"))*100
...
def makeRequest(request):
    r = requests.post('https://sanctionssearch.ofac.treas.gov/', 
            data = {
                ...
                'ctl00$MainContent$txtLastName':'joshua',
